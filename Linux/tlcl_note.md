...
**bash** 是 "Bourne Again SHell" 的首字母缩写，是最初 Unix 上由 Steve Bourne 写的 shell 程序 `sh` 的增强版
...
X 窗口系统（使 GUI工作的底层引擎）内建了一种机制，支持 __快速拷贝__ 和 __粘贴技巧__ :按下鼠标左键，沿着文本拖动鼠标（或者双击一个单词）高亮了一些文本，那么这些高亮的文本就被拷贝到了一个由 X 管理的缓冲区里面。然后按下鼠标中键，这些文本就被粘贴到光标所在的位置。

不要在一个终端窗口里使用 `Ctrl-c` 和 `Ctrl-v` 快捷键来执行拷贝和粘贴操作,这两个控制代码有着不同的含义。使用 `Ctrl-Shift-c` 和 `Ctrl-Shift-v` 来进行复制粘贴。
...
命令中的选项是由一个 **中划线** 加上 **一个字符** 组成，例如 `-l`，但是许多命令支持长选项，长选项由 **两个中划线** 加上 **一个字(word)** 组成。许多命令也允许把多个短选项串在一起使用。

### `less` 命令

`less` 命令是一个用来浏览文本文件的程序。如果文件内容多于一页，那么我们可以上下滚动文件。按下 `q` 键，退出程序。
...
传递到 `echo` 命令的任一个参数都会在(屏幕上)显示出来，`-n` 选项表示不换号。
...
命令替换允许我们把一个命令的输出作为字符串来使用，有两种引用方式。
...
```
花括号展开模式可能包含一个开头部分叫做报头，一个结尾部分叫做附言。花括号表达式 本身可能包含一个由逗号分开的字符串列表，或者一个整数区间，或者单个的字符的区间。
...
`cat` 接受多个文件作为参数，然后拼接。常用于连接被分散的文件。
...
`tee` 从 标准输入 读取数据，并同时输出到 标准输出 和文件。
